...
def gradescent(data, p_initial, tol=1e-1):
    p = p_initial
    l = 2 # learning rate
    ...
    while ~converged and (i < 1000):
        ...
        i += 1
    return np.array(p).astype(int), converged
...
p, converged = gradescent(data, p_initial, tol=0.1)
plotresult(data_ori, p)
