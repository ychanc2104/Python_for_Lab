...
def plot_EM_result(result, ax, xlabel='iteration', ylabel='value'):
    ...
    for i in range(n_feature):
        ax.plot(np.arange(0, j + 1), result[:, i], '-o')
...
def reshape_all(*args, n_feature):
    results = []
    n_sample = int(len(args[0])/n_feature)
    for arg in args:
        results += [np.reshape(arg, (n_sample, n_feature))]
    return results
...
while (j < 50 or improvement > tolerance) and j < 500:
    ...
    improvement = cal_improvement([m_save[-n_feature:], m], [s_save[-n_feature:], s], [f_save[-n_feature:], f])
    j += 1
m_save, s_save, f_save = collect_EM_results([m_save, m], [s_save, s], [f_save, f])
m_save, s_save, f_save = reshape_all(m_save, s_save, f_save, n_feature=n_feature)
label = get_label(data, likelihood)
data_cluster = [data[label==i] for i in range(n_feature)]
...
y_fit = np.array([f[i]*oneD_gaussian(x, m[i], s[i]) for i in range(n_feature)])
for i in range(n_feature):
    plt.plot(x, y_fit[i,:], '-')
